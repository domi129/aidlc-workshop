...
var import_legacy = __toESM(require("neotraverse/legacy"));
...
function mock(service, method, replace) {
  if (!services[service]) {
    const service_to_add = {
      ...
      Constructor: (0, import_legacy.default)(_AWS).get(service.split(".")),
      methodMocks: {},
      invoked: false
    };
...
function mockService(service) {
  ...
  const object = (0, import_legacy.default)(_AWS).get(nestedServices);
  const service_obj = services[service];
  if (service_obj) {
...
function mockServiceMethod(service, client, method, replace) {
  ...
  serviceMethodMock.stub = import_sinon.default.stub(client, method).callsFake(function(...args) {
    let userArgs;
    let userCallback;
